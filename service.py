...
import json
import time
from app.larkcustombot.robot_arsenal import RobotArsenal
from app.larkcustombot.service_roll import ServiceRoll
...
class Service:
    ...
    def __init_no_disturb_dict(self):
        ...
        # 免打扰用户字典 {服务 -> {群组 -> [用户], }, }
        self.no_disturb_dict = json.loads(fp)
    ...
    def process_message_service(self, request: dict):
        ...
        print(
            "[ServiceCenter][{0}] 服务处理{1}，请求内容{2}".format(time_str, result and "成功" or "失败", str(request)))
        return
