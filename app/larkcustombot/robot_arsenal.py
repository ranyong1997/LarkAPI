...
class RobotArsenal:
    ...
    def __get_user_id_with_name(self, username) -> str:
        ...
        if username in self.name_to_id_dict.keys():
            ...
            if username in self.name_to_id_dict.keys():
                ...
                print(
                    '[RobotArsenal.__get_user_id_with_name] 机器人所在部门找不到用户名为\'{0}\'的成员'.format(username))
                return None
    ...
    def __preprocess_rich_message(self, raw_message: str) -> list:
        ...
        if len(message) != len(raw_message):
            print(
                "[RobotArsenal.__preprocess_rich_message] 处理富文本时删减了部分文本，发送内容为:\n\033[1;33m{0}\033[0m".format(
                    message))
